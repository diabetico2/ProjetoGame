...
export const getDailyRanking = async (req: Request, res: Response): Promise<void> => {
  try {
    let date: Date;
...
export const getOverallRanking = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const formattedRanking = overallRanking
      .map(user => ({
        idJogador: user.id,
        nome: user.name,
        melhorPontuacao: user.matches[0]?.score || 0,
...
export const getPlayerTopMatches = async (req: Request, res: Response): Promise<void> => {
  try {
    const { userId } = req.params;
